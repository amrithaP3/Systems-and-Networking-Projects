...
SKP_UNUSED_SIZE = 19
assert (
    OPCODE_WIDTH + SKP_MODE_BITS + REGISTER_WIDTH * 2 + SKP_UNUSED_SIZE == BIT_WIDTH
)  # Sanity check
    ...
    if bin_offset is None:
        ...
        if unsigned:
            ...
            if offset < 0:
                ...
                raise RuntimeError(
                    "'{}' is too large (as a value) or too far away (as a label) for {}.".format(
                        offset, __name__
                    )
                )
        else:
            ...
            if offset < -bound:
                raise RuntimeError(
                    "'{}' is too small (as a value) or too far away (as a label) for {}.".format(
                        offset, __name__
                    )
                )
            elif offset >= bound:
...
class RInstruction(Instruction):
    ...
    __RE_R = re.compile(
        r"^\s*(?P<RX>\$\w+?)\s*,\s*(?P<RY>\$\w+?)\s*,\s*(?P<RZ>\$\w+?)(,\s*(?P<ShfType>LSL|LSR|ASR)\s+(?P<ShfDist>\d+))?\s*$",
        re.I,
    )
...
class addi(Instruction):
    __RE_ADDI = re.compile(
        r"^\s*(?P<RX>\$\w+?)\s*,\s*(?P<RY>\$\w+?)\s*,\s*(?P<Offset>\S+?)\s*(,\s*(?P<ShfType>LSL|LSR|ASR)\s+(?P<ShfDist>\d+))?$",
        re.I,
    )
...
class beq(Instruction):
    __RE_BEQ = re.compile(
        r"^\s*(?P<RX>\$\w+?)\s*,\s*(?P<RY>\$\w+?)\s*,\s*(?P<Offset>\S+?)\s*$"
    )
...
class minmax(RInstruction):
    ...
    def parse_operands(cls, operands, pc, instruction):
        ...
        if instruction == "min":
            ...
            raise RuntimeError(f"Instruction '{instruction}' is not valid.")
        match = getattr(cls, f"_{RInstruction.__name__}__RE_R").match(operands)
        # match = cls.__RE_R.match(operands) # doesn't work bc cls is minmax and not RInstruction so python looks up __minmax_RE_R instead of __RInstruction_RE_R.
        ...
        result_list.append(parse_register(match.group("RY")))
        # Insert unused bits
        result_list.append(zero_extend(mode, R_UNUSED_SIZE))
        result_list.append(parse_register(match.group("RZ")))

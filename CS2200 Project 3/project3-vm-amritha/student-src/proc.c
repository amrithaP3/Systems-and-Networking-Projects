...
void proc_init(pcb_t *proc) {
    ...
    // Getting pfn for a new freed frame for page table
    pfn_t victim_pfn = free_frame();
    
    pte_t *page_table = (pte_t *) (mem + victim_pfn * PAGE_SIZE);
    memset(page_table, 0, PAGE_SIZE);

    // Updating the frame table
    frame_table[victim_pfn].protected = 1;
    frame_table[victim_pfn].mapped = 1;
    frame_table[victim_pfn].process = proc;

    // Updating the process's PCB
...
void proc_cleanup(pcb_t *proc) {
    ...
    for (size_t i = 0; i < NUM_PAGES; i++) {
        ...
        // Freeing swap entry if it exists
        if (swap_exists(entry)) {
            swap_free(entry);
        }
    }

    // Clearing protected bit if set

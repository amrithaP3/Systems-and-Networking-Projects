            ...
            for (vpn = 0; vpn < NUM_PAGES; vpn++)
            {
                ...
                if (pgtable[vpn].valid) {
                    ...
                    if (mapped_frames_accounted_for[found_pfn]) {
                        panic("Multiple valid page table entries map to same PFN");
                    }

                    if (frame_table[found_pfn].process < procs
                        || frame_table[found_pfn].process >= procs + MAX_PID) {
                        panic("Frames should not be owned by invalid or terminated processes");
                    }

                    /* Check that frame table agrees with page table */
                    if (!frame_table[found_pfn].mapped) {
                        panic("Valid page table entry points to unmapped physical frame");
                    }

                    if(frame_table[found_pfn].process->pid != pid){
                        panic("Process owning valid page table entry does not match process owning corresponding PFN")
                    }

                    if(frame_table[found_pfn].vpn != vpn) {
                        panic("Valid VPN to PFN mapping in page table does not match PFN to VPN mapping in frame table")
                    }
...
void print_help_and_exit()
{
    ...
    printf("  -r\t\tSelect the replacement algorithm (either 'random' or 'lru' or 'clocksweep')\n");
